...
// 触摸屏实际就是校准这些值 
	...
	#define TOUCH_AD_TOP            2016     //YPhys0
	#define TOUCH_AD_BOTTOM         56       //YPhys1
...
static void LcdWriteReg(U16 Data) {
  // ... TBD by user
	BSP_LCD_CMD  = Data;	//modify by fire
}
...
static void LcdWriteData(U16 Data) {
  // ... TBD by user								
	BSP_LCD_DAT = Data;//modify by fire
}
...
static void LcdWriteDataMultiple(U16 * pData, int NumItems) {
  while (NumItems--) {
    // ... TBD by user
	BSP_LCD_DAT =  *pData++;//modify by fire
  }
}
...
static void LcdReadDataMultiple(U16 * pData, int NumItems) {
  ...
  while (NumItems--) {
    // ... TBD by user
		*pData++ = BSP_LCD_DAT;					//modify by fire
...
/*设置触摸校准 */	
  ...
  GUI_TOUCH_Calibrate(GUI_COORD_X, 0, 320-1, TOUCH_AD_RIGHT ,TOUCH_AD_LEFT );
  GUI_TOUCH_Calibrate(GUI_COORD_Y, 0, 240-1, TOUCH_AD_BOTTOM ,TOUCH_AD_TOP );
