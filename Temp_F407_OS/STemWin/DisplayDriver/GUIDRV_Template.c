...
// 1表示优化相应的接口函数，当不优化时所有操作都调用打点函数，效率可想而知了
...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      // TBD by customer...
      //
		BSP_LCD_SetPointPixel(xPhys, yPhys, PixelIndex);
    }
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      // TBD by customer...
      //
		BSP_LCD_GetPointPixel(xPhys, yPhys);
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    for (; x0 <= x1; x0++) {
      _XorPixel(pDevice, x0, y);
		...
		BSP_LCD_DrawHLine(x0, y, x1, ColorIndex);
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    for (; y0 <= y1; y0++) {
      _XorPixel(pDevice, x, y0);
		...
		BSP_LCD_DrawVLine(x, y0, y1, ColorIndex);
	#else
		ColorIndex = LCD__GetColorIndex();
	    for (; y0 <= y1; y0++) {
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
	...
	else
	{
	  /* 选择进行优化 */
		...
		BSP_LCD_FillRect(x0, y0, y1-y0+1, x1-x0+1, ColorIndex);
	#else
		for (; y0 <= y1; y0++) 
		{
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const * p, int xsize) {
		...
		BSP_LCD_DrawHColorLine(x, y, xsize, (uint16_t *)p);
